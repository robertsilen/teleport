...
Configure Teleport to create local users when Teleport users start an SSH session.
...
## Static host user creation

Configure Teleport to create local users independently of an SSH session.
...
be created. If your static host user has more than one matcher, a server can't
be matched by more than one of them.
...
Connect to the remote Node via `tsh` and see that the host user was created:

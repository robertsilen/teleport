...
func TestServiceAccess(t *testing.T) {
	...
	t.Run("verify coverage", func(t *testing.T) {
		for _, method := range userintegrationtasksv1.UserIntegrationTaskService_ServiceDesc.Methods {
...
// callMethod calls a method with given name in the UserIntegrationTask service
func callMethod(t *testing.T, service *Service, method string) error {
	for _, desc := range userintegrationtasksv1.UserIntegrationTaskService_ServiceDesc.Methods {
...
func (f fakeChecker) CheckAccessToRule(_ services.RuleContext, _ string, resource string, verb string) error {
	if resource == types.KindUserIntegrationTask {
...
func newService(t *testing.T, checker services.AccessChecker) *Service {
	...
	backendService, err := local.NewUserIntegrationTasksService(b)
	require.NoError(t, err)

...
func (c *ServiceConfig[T]) CheckAndSetDefaults() error {
	...
	if c.KeyFunc == nil {
		c.KeyFunc = func(t T) backend.Key { return backend.NewKey(t.GetName()) }
	}
...
func (s *Service[T]) WithPrefix(parts ...string) *Service[T] {
	...
	return &Service[T]{
		...
		backendPrefix:               s.backendPrefix.AppendKey(parts),
...
func (s *Service[T]) listResourcesReturnNextResourceWithKey(ctx context.Context, pageSize int, pageToken string) ([]T, *T, string, error) {
	...
	if len(out) > pageSize {
		...
		out = out[:pageSize]
		nextKey = lastKey.TrimPrefix(s.backendPrefix).String()[len(backend.Separator):]
	}
...
func (s *Service[T]) ListResourcesWithFilter(ctx context.Context, pageSize int, pageToken string, matcher func(T) bool) ([]T, string, error) {
	rangeStart := s.backendPrefix.AppendKey(backend.NewKey(pageToken))
	rangeEnd := backend.RangeEnd(s.backendPrefix.ExactKey())
...
func (s *Service[T]) UpdateAndSwapResource(ctx context.Context, name string, modify func(T) error) (T, error) {
	var t T
	key := backend.NewKey(name)
	existingItem, err := s.backend.Get(ctx, s.MakeKey(key))
	if err != nil {
		if trace.IsNotFound(err) {
	...
	replacementItem, err := s.MakeBackendItem(resource, key)
	if err != nil {
		return t, trace.Wrap(err)
	}
...
func (s *Service[T]) MakeBackendItem(resource T, k backend.Key) (backend.Item, error) {
...
func (s *Service[T]) RunWhileLocked(ctx context.Context, lockName backend.Key, ttl time.Duration, fn func(context.Context, backend.Backend) error) error {
